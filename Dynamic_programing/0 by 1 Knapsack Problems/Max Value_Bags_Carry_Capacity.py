...
"""
A man has the capacity to lift only C kilograms. There are N bags containing grocery items to be carried to yet another location. 
The weight of the bags are distinct (that is the weight of each bag is different from yet another bag). There is a price tag on each bag denoting the value of the bag. 
The program must calculate the maximum value that can be transported by the man in his first trip considering the fact that he can lift only C kilograms in a trip. 
Then the program must print the weight of the bags (in the same order of input) to be carried so that the value carried in the first trip is maximum.
